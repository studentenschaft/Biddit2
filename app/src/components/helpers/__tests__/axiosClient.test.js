...
import { server, setErrorMode, resetErrorMode, ErrorSimulation } from '../../../test/mocks/server';
...
describe('ApiClient Error Handling', () => {
  ...
  describe('401 Unauthorized Handling', () => {
    ...
    it('should trigger session expired when token refresh fails', async () => {
      ...
      const handleAuthFailureSpy = vi.spyOn(tokenService, 'handleAuthFailure').mockImplementation(() => {});
  ...
  describe('Network Error Handling', () => {
    it('should emit OFFLINE event when offline', async () => {
      const events = [];
      const unsubscribe = addNetworkEventListener((event) => events.push(event));
