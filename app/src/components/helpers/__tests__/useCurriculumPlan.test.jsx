...
const compareSemesters = (a, b) => {
  ...
  if (parsedA.fullYear !== parsedB.fullYear) return parsedA.fullYear - parsedB.fullYear;
...
describe('useCurriculumPlan hook integration', () => {
  ...
  it('moveCourse rejects completed semesters', async () => {
    ...
    const success = result.current.moveCourse('ABC123', 'FS26', 'HS25', 'Core', 'wishlist');
  ...
  it('moveCourse returns true for same semester (no-op)', async () => {
    ...
    const success = result.current.moveCourse('ABC123', 'FS26', 'FS26', 'Core', 'wishlist');
