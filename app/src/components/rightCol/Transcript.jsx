...
// Import unified course data selectors
...
const Transcript = () => {
  ...
  const allCourseInfo = useRecoilValue(allSemesterCoursesSelector); // For filtering available courses - using unified data

  // Get current semester for UI
  const currentSemester = useCurrentSemester();

  // Simple fallback for semester index since we no longer use cisIdList
  // The GradeTranscript component will use this for display ordering
  const currentSemesterIndex = 0; // Default to 0 for now
  ...
  const wishlistCourses = useMemo(() => {
    if (
      ...
      Object.keys(mainProgramStudyPlan).length === 0
    ) {
      return [];
    }
    ...
    Object.entries(mainProgramStudyPlan).forEach(([semesterLabel, courses]) => {
      ...
      courses.forEach((course) => {
        ...
        if (course.type?.endsWith("-wishlist") || !course.grade) {
          ...
          if (
            availableCourseNumbers.has(course.id) ||
            ...
            if (
              ...
              wishlistCourse.classification =
                enrichmentData.classification || course.classification;
              wishlistCourse.description =
                enrichmentData.description || course.description;
            }
    ...
    return wishlist;
  }, [mainProgramStudyPlan, allCourseInfo]); // Course data dependencies only
  ...
  const buildWishlistMapping = useCallback((wishlistCourses) => {
    ...
    console.log(
      "🔍 Transcript: Available classification keys:",
      Object.keys(mapping)
    );
    return mapping;
  }, []);
   ...
   */
  const mergeWishlistIntoTranscript = useCallback(
    (transcript, wishlistCourses) => {
          ...
          if (item.isTitle) {
            ...
            Object.keys(wishlistMap).forEach((key) => {
              ...
              const isVariationMatch =
                ...
                (normalizedKey === "elective" &&
                  normalizedCategoryName === "electives") ||
                (normalizedKey === "electives" &&
                  normalizedCategoryName === "elective");

              if (isExactMatch || isVariationMatch) {
                console.log(
                  `🎯 Transcript: Matching "${key}" courses to "${categoryName}" section`
                );
                ...
                wishlistMap[key].forEach((course) => {
                  ...
                  console.log(
                    `📝 Transcript: Created course item for removal:`,
                    {
                      id: newCourseItem.id,
  ...
  const mergedTranscript = useMemo(() => {
    if (mainProgramTranscript && wishlistCourses.length > 0) {
      return mergeWishlistIntoTranscript(
        mainProgramTranscript,
        wishlistCourses
      );
    }
  ...
  const isLoading =
    !initStatus.isInitialized || !currentProgramData || !mainProgramTranscript;
